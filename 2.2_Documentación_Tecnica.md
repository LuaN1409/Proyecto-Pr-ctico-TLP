...
* **Empaquetado**: disponible un ejecutable en `dist/` para correr la demo sin instalar Python.

Carpetas/archivos clave del repositorio:

* `analizador.py` (Tokenizer + Parser) → produce `arbol.ast`
* `arbol.ast` → ejemplo de salida
...
Archivo `ejemplo.brik`:

```
juego = "snake"
titulo = "Demo Snake"
pantalla = { ancho = 640, alto = 480 }
```
...
## 5. `motor_runtime.py` — el motor, explicado paso a paso
...
## 6. `input.py` — gestión de entradas, explicado fácil
...
## 7. `render.py` — renderizado con `tkinter`, explicado paso a paso
...
* El ejecutable en `dist/` fue creado con PyInstaller. Windows puede advertir porque el `.exe` no está firmado. Si se prefiere evitar advertencias, ejecutar el script con Python en modo desarrollo.
